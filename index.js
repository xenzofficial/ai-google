...
const WebSocket = require('ws'); // Tambahkan library ws
...
// Middleware untuk parsing JSON body (masih berguna jika ada endpoint HTTP lain)
app.use(express.json());

// Buat HTTP server dari Express app
const server = http.createServer(app);

// Buat WebSocket server yang menempel pada HTTP server
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      console.error("Failed to parse message or missing prompt:", message);
    ...
    ws.send(JSON.stringify({ status: "Processing your request..." })); // Kirim status awal
    ...
    if (userMessageIndex !== -1) {
      ...
      if (requestData.messages[1] && requestData.messages[1].role === "user") {
          ...
          ws.send(JSON.stringify({ error: "Internal server error: Could not structure user prompt." }));
          return;
      }
    }
    ...
    const options = {
      ...
      headers: {
        ...
        'x-request-id': `ws-${Date.now()}-${Math.random().toString(36).substring(7)}`, // Generate unik per request
      ...
      // Tidak ada timeout untuk body fetch di sini, karena WebSocket sudah menangani koneksi jangka panjang.
      // Timeout AbortController tetap berguna untuk permintaan spesifik ke Qwen API.
      body: JSON.stringify(requestData)
    };

    const qwenApiUrl = 'https://chat.qwen.ai/api/chat/completions';
    const requestTimeout = 10 * 60 * 1000; // 10 menit untuk fetch ke Qwen, bisa disesuaikan
    ...
    try {
      ...
      if (qwenResult.choices && qwenResult.choices.length > 0 && qwenResult.choices[0].message && qwenResult.choices[0].message.content) {
        ...
        ws.send(JSON.stringify({ error: "Gagal memproses respons dari Qwen API. Struktur tidak dikenali." }));
      }

    } catch (error) {
      clearTimeout(timeoutId);
      if (error.name === 'AbortError') {
        ws.send(JSON.stringify({ error: "Permintaan ke Qwen API timeout." }));
      } else {
        console.error("Error saat menghubungi Qwen API:", error);
        ws.send(JSON.stringify({ error: "Terjadi kesalahan internal saat menghubungi Qwen API.", details: error.message }));
      }
    }
  });
...
app.get('/', (req, res) => {
  res.send('Qwen API Proxy with WebSocket is running!');
});

// Timeout server HTTP. Untuk WebSocket, koneksi bisa lebih lama.
const serverTimeout = 15 * 60 * 1000; // 15 menit server timeout untuk HTTP
server.listen(PORT, () => {
